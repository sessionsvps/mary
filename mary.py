...
async def logUserBot():
    load_dotenv()
    api_id = int(24067355)
    api_hash = "b64556aee52c421491b865cb4a4a8102"
    phone_number = "51970028557"
    session_name = "mary_spammer"
    client = TelegramClient(session_name, api_id, api_hash)
    await client.connect()
    if not await client.is_user_authorized():
        await client.send_code_request(phone_number)
        await client.sign_in(phone_number, input('Ingrese el código de verificación: '))
    await client.send_message("@spam13i18r", f'<b>Bot encendido</b>', parse_mode="HTML")
    spammer_group = int("-4165892218")
    spammer_group2 = int("-4283472860")
    ...
    while True:
        ...
        try:
            await client.send_message("@spam13i18r", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>",parse_mode="HTML")
        except:
            pass
            
        try:
            for i in groups_info:
                if i['group_name'] not in ["Spam Especial","Spam 2024","RESPALDO🇵🇪BINS PERU","➳𝒀𝑨𝑷𝑬 𝑫𝑬 𝑬𝑺𝑻𝑨𝑭𝑨𝑫𝑶𝑹𝑬𝑺 ✧","QUEMANDO ESTAFADORES","𝐏𝐄𝐑Ú 𝐀𝐘𝐔𝐃𝐀","Referencias Elmer 💸","🎭 CANAL MUNDO STREAMING PERÚ 🇵🇪","TU MARKETPLACE"]:
                    ...
                    if i['group_name'] in ["🇵🇪🧑🏼‍💻𝙋𝙀𝙍𝙐 𝘼𝙔𝙐𝘿𝘼🧑🏼‍💻🇵🇪","BINS PERU🇵🇪","COMPRA Y VENTA🇵🇪PERU","STREAMING LATINO GO","🇵🇪🧑🏼‍💻𝘽𝙐𝙎𝙌𝙐𝙀𝘿𝘼𝙎 𝙋𝙀𝙍𝙐🧑🏼‍💻🇵🇪"]:
                        ...
                        for message_spam in special_messages_list:
                            ...
                            try:
                                ...
                                await client.send_message("@spam13i18r", f'<b>Error enviando mensajes a  a {i["group_id"]}</b> - <code>{i["group_name"]}<code>\nCausa:{error}',parse_mode="HTML")
                                resultado = False
                            if resultado:
                                await client.send_message("@spam13i18r", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>',parse_mode="HTML")  
                        ...
                        for message_spam in messages_list:
                            ...
                            if j==4: break
            await client.send_message("@spam13i18r", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
